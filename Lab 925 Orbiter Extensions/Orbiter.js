...
Orbiter.prototype.connect=function(){
    for(let i=0;i<movers.length;i++){
        for(let j=0;j<this.mover.connections.length;j++){
            if(movers[i]===this.mover.connections[j]){
                return;
            }
        }
        if(movers[i]!==this&&this.loc.distance(movers[i].loc)<2*this.oRad){
            this.mover.connections.push(movers[i]);
        }
    }
}